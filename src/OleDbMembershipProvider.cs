...
namespace Binarywater.NET.ASP.Providers
{

    public sealed class OleDbMembershipProvider : MembershipProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (String.IsNullOrEmpty(config["description"]))
            {
                ...
                config.Add("description", "OleDb Membership provider");
            }
                 ...
                 out MembershipCreateStatus status)
        {
            ...
            if (u == null)
            {
                ...
                OleDbConnection conn = new OleDbConnection(connectionString);
                //OleDbCommand cmd = new OleDbCommand("INSERT INTO [" + tableName + "]" +
                //      " (PKID, Username,[Password],Email,CreationDate)" +
                //      " Values(?,?,?,?,?)", conn);
        ...
        {
            OleDbConnection conn = new OleDbConnection(connectionString);
            OleDbCommand cmd = new OleDbCommand("SELECT Count(*) FROM [" + tableName + "] " +
        ...
        public override int GetNumberOfUsersOnline()
        {
            ...
            catch (OleDbException e)
            {
                if (WriteExceptionsToEventLog)
                {
                    ...
                    throw new ProviderException(exceptionMessage+" "+e.StackTrace);
                }
                else
                {
                    throw e;
        ...
        public override string GetPassword(string username, string answer)
        {
            ...
            OleDbConnection conn = new OleDbConnection(connectionString);
            OleDbCommand cmd = new OleDbCommand("SELECT [Password], PasswordAnswer, IsLockedOut FROM [" + tableName + "]" +
                  " WHERE Username = ? AND ApplicationName = ?", conn);
        ...
        {
            OleDbConnection conn = new OleDbConnection(connectionString);
            OleDbCommand cmd = new OleDbCommand("SELECT PKID, Username, Email, PasswordQuestion," +
                 " Comment, IsApproved, IsLockedOut, CreationDate, LastLoginDate," +
                 " LastActivityDate, LastPasswordChangedDate, LastLockedOutDate" +
            ...
            try
            {
                ...
                if (reader.HasRows)
                {
                    ...
                    if (userIsOnline)
                    {
                        ...
                        updateCmd.Parameters.Add("@LastActivityDate", OleDbType.Date).Value = DateTime.Now;
                        updateCmd.Parameters.Add("@PKID", OleDbType.Guid).Value = providerUserKey;
        ...
        public override string ResetPassword(string username, string answer)
        {
            ...
            OleDbConnection conn = new OleDbConnection(connectionString);
            OleDbCommand cmd = new OleDbCommand("SELECT PasswordAnswer, IsLockedOut FROM [" + tableName + "]" +
                  " WHERE Username = ? AND ApplicationName = ?", conn);
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            //System.Data.OleDb.OleDbConnection x = new System.Data.OleDb.OleDbConnection(connectionString);
            //x.Open();


            OleDbCommand cmd = new OleDbCommand("SELECT [Password], IsApproved FROM [" + tableName + "]" +
                    " WHERE Username = ? AND ApplicationName = ? AND IsLockedOut = False", conn);
            ...
            try
            {
                ...
                if (CheckPassword(password, pwd))
                {
                    if (isApproved)
                    {
                        ...
                        OleDbCommand updateCmd = new OleDbCommand("UPDATE [" + tableName + "] SET LastLoginDate = ?" +
                                                                " WHERE Username = ? AND ApplicationName = ?", conn);
        ...
        private byte[] HexToByte(string hexString)
        {
            ...
            int opt = hexString.IndexOf(",");

            if (opt > 0)
            {
                hexString = hexString.Substring(0, opt);
            }
